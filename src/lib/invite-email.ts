...
export async function sendInviteEmail(
  ...
  <div style="font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,sans-serif;background:#f5f5f4;padding:32px 16px">
    <div style="max-width:520px;margin:0 auto;background:#fff;border:1px solid #e7e5e4;border-radius:14px;overflow:hidden">
      <div style="background:#1c1917;color:#fff;padding:24px 28px">
        ...
        <div style="font-size:13px;color:#a8a29e;margin-top:2px">Klantportal</div>
      </div>
      <div style="padding:28px">
        <h1 style="margin:0 0 8px;font-size:20px;color:#1c1917">Stel je account in</h1>
        <p style="margin:0 0 22px;font-size:14px;line-height:1.6;color:#57534e">
          We hebben portaltoegang voor je klaargezet. Daar kun je je facturen
          bekijken wanneer je maar wilt.
        </p>
        ...
        <p style="margin:22px 0 0;font-size:13px;line-height:1.6;color:#78716c">
          Deze link werkt eenmalig en verloopt na 14 dagen. Als je dit niet
          verwachtte, kun je deze e-mail negeren — er is niets aangemaakt
          waarvoor je kosten in rekening worden gebracht.
        </p>
      </div>
    </div>
  </div>`;
  ...
  const text = [
    `${SITE.name} — klantportal`,
    ``,
    `We hebben portaltoegang voor je klaargezet. Kies hier je wachtwoord:`,
    link,
    ``,
    `Deze link werkt eenmalig en verloopt na 14 dagen.`,
  ].join("\n");

  if (!resend) {
    console.warn(`[invite-email] RESEND_API_KEY niet ingesteld — uitnodiging voor ${to} is niet gemaild.`);
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      subject: `${SITE.name}: stel je klantportal-account in`,
      html,
      text,
    });
    if (error) {
