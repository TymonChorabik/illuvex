...
function customerEmail(order: Order) {
  ...
  <div style="font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,sans-serif;background:#f5f5f4;padding:32px 16px">
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e7e5e4;border-radius:14px;overflow:hidden">
      ...
      <div style="padding:28px">
        <h1 style="margin:0 0 8px;font-size:20px;color:#1c1917">Thanks, ${escapeHtml(order.name)} — we've got your request.</h1>
        <p style="margin:0 0 22px;font-size:14px;line-height:1.6;color:#57534e">
          This is your confirmation that we received your request for the
          <strong>${escapeHtml(order.offerName)}</strong> package. We'll reply within one
          working day with next steps and a start date.
        </p>

        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:22px">
          <tr>
            <td style="padding:9px 0;color:#78716c;border-bottom:1px solid #f5f5f4">Reference</td>
          ...
          <tr>
            <td style="padding:9px 0;color:#78716c;border-bottom:1px solid #f5f5f4">Package</td>
          ...
          <tr>
            <td style="padding:9px 0;color:#78716c;border-bottom:1px solid #f5f5f4">Price</td>
            <td style="padding:9px 0;text-align:right;font-weight:600;color:#1c1917;border-bottom:1px solid #f5f5f4">${escapeHtml(price)}</td>
          </tr>
          <tr>
            <td style="padding:9px 0;color:#78716c">Typical turnaround</td>
            <td style="padding:9px 0;text-align:right;font-weight:600;color:#1c1917">${escapeHtml(offer?.timelineLabel ?? "We'll confirm")}</td>
          </tr>
        </table>
        ...
        ${
          includes
            ? `<div style="background:#fafaf9;border:1px solid #f5f5f4;border-radius:10px;padding:16px 18px;margin-bottom:22px">
                 <div style="font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#78716c;margin-bottom:10px">What's included</div>
        ...
        ${
          order.notes
            ? `<div style="margin-bottom:22px">
                 <div style="font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#78716c;margin-bottom:6px">Your notes</div>
        ...
        <p style="margin:0;font-size:13px;line-height:1.6;color:#78716c">
          Nothing is charged yet — this is a request, not an invoice. Reply to this
          email if anything needs changing.
        </p>
      </div>
    </div>
  </div>`;
  ...
  const text = [
    `Thanks, ${order.name} — we've got your request.`,
    ``,
    `Reference: ${order.reference}`,
    `Package:   ${order.offerName}`,
    `Price:     ${price}`,
    `Turnaround: ${offer?.timelineLabel ?? "we'll confirm"}`,
    ``,
    order.notes ? `Your notes: ${order.notes}\n` : ``,
    `We'll reply within one working day. Nothing is charged yet.`,
    ``,
    `— ${SITE.name}`,
  ].join("\n");
...
function ownerEmail(order: Order) {
  ...
  <div style="font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,sans-serif;padding:24px">
    <h2 style="margin:0 0 4px;font-size:18px">New request: ${escapeHtml(order.offerName)}</h2>
...
export async function sendOrderEmails(order: Order): Promise<EmailResult> {
  ...
  if (!resend) {
    console.warn(
      `[email] RESEND_API_KEY is not set — confirmation for ${order.reference} was not sent.`,
    );
    console.info(`[email] Would have sent to ${order.email}:\n${customer.text}`);
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      subject: `${SITE.name}: we've got your ${order.offerName} request (${order.reference})`,
      html: customer.html,
      text: customer.text,
    });
    ...
    const notification = await resend.emails.send({
      ...
      replyTo: order.email,
      subject: `New ${order.offerName} request — ${order.name} (${order.reference})`,
      html: owner.html,
      text: owner.text,
    });
